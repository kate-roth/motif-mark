...
def draw_genes_motifs(gene_dict, motif_ls, ctx, cols):
    ...
    for g in gene_dict.values():
        # set positions
        gene_pos = (gene_n*height_per_gene)-(height_per_gene/2)
        label_pos = gene_pos - (height_per_gene/4)
        ...
        ctx.move_to(left_margin, gene_pos)
        ctx.line_to((left_margin+g[2][3]), gene_pos)
        ctx.stroke()

        # draw exon
        ctx.set_line_width(feature_height)
        ctx.move_to((left_margin+g[2][0]), gene_pos)
        ctx.line_to((left_margin+g[2][0]+g[2][1]), gene_pos)
        ctx.stroke()
        ...
        for i in range(len(motif_ls)):
            ...
            for n in mtfs.values():
                for o in n:     # draw motifs, different motifs have different colors
                    ...
                    ctx.set_line_width(feature_height)
                    ctx.move_to((left_margin+o[0]), gene_pos)
                    ctx.line_to((left_margin+o[1]), gene_pos)
                    ctx.stroke()
        genes_motifs[g[0]] = all_motifs
    return(genes_motifs)
...
motifs_by_gene = draw_genes_motifs(genes, motif_pat, context, dec_colors)
